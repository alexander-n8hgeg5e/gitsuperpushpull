...
from os.path import normpath,sep as psep,expanduser as f_expanduser ,expandvars as f_expandvars
import neovim
import logging
from re import sub
...
def parse_args(*z,**zz):
    ...
    parser.add_argument('-t', '--ssh_target',default=None, action="store" ,help='string that defines the other host and that is configured for ssh like foo in "ssh foo"')
    ...
    parser.add_argument('-u', '--url', action="store", default='' ,help='url to use for submodule url generation or path for --cmd or --clone')
    parser.add_argument('-f', '--file', action="store" , type=str,default=None, help='path to file with .git dirs listed')
    parser.add_argument('-a', '--urlpostfix', action="store" , default="" ,help='append url postfix')
    parser.add_argument('-c', '--cut-path',  action="store" , default=0 , type=int ,help='cut this many path components from path for url')
    parser.add_argument('-r', '--remote-name', action="store" , default=None ,help='name of remote')
    parser.add_argument('-D', '--mkdir', action="store_true" , default=False ,help='option for -C/--cmd : create dir if not existing')
    parser.add_argument('-C', '--cmd', action="store" , default=None ,help='do cmd in gitdirs specified in file')
    ...
    parser.add_argument('-o', '--clone', action="store_true" ,help='clone to -t dir as basedir , into dirs from -f that are cutted with -c and prefixed by -u,and postfixed by -a, respects dryrun' )
    parser.add_argument('-x', '--fix', action="store_true" ,help='fix more ask less')
    parser.add_argument('-N', '--dry-run', action="store_true" ,help='works only for "-C/--cmd"')
    ...
    return parser.parse_args(*z,**zz)

def init(argz=None):
    global LOGLEVEL
    global args
    global args_
    args=parse_args(args=argz)
    args_=args
...
def process_entry(dotgitdir):
    if not checkr_create_git_remote_entry(dotgitdir,sshhost_string):
        ...
        while not (check_push(repopath) or retrycount == 10):
            ...
            if not offer_fixes(dotgitdir,checkr_create_git_remote_entry,dotgitdir,sshhost_string):
                break
...
def check_git_remote_entry_name(dotgitdir,sshhost_string):
    """
    ...
    try:
        ...
        log.error("ERROR: fish code failed: "+fish_script)
...
def checkr_create_git_remote_entry(dotgitdir,sshhost_string,check_remote=False):
    """
    ...
    """
    repopath = remove_trailing_somewhat_from_path(dotgitdir)
    foundname = check_git_remote_entry_name(dotgitdir,sshhost_string)
    if not foundname:
      message("REMOTE ENTRY MISSING: local gitdir: "+dotgitdir)
      if args.fix:
          if check_create_git_remote_entry(repopath,sshhost_string):
              # additional check
              return checkr_create_git_remote_entry(dotgitdir,sshhost_string,check_remote=check_remote)
      else:
          if check_offer_fixes( dotgitdir,check_ask_create_git_remote_entry, checkr_create_git_remote_entry, ([repopath],None), ([dotgitdir,sshhost_string],{"check_remote":check_remote}) ):
                # additional check
                return checkr_create_git_remote_entry(dotgitdir,sshhost_string,check_remote=check_remote)
    if foundname:
        ...
        if not remote_entry_ok:
          ...
          if args.fix:
              ...
              if check_offer_fixes( dotgitdir, check_ask_remove_git_remote_entry, checkr_create_git_remote_entry, ([repopath],None), ([dotgitdir,sshhost_string],{"check_remote":check_remote}) ):
                  #additional check
                  return checkr_create_git_remote_entry(dotgitdir,sshhost_string,check_remote=check_remote)
        else:
            # git 'remote entry' ok
...
def check_create_git_remote_entry(repopath,sshhost_string,remote_repo_path=None):
    """
    Add the remote entry to the the specified repo.
    """
    log.debug( "creating remote entry in : "+ repopath )
    ...
    if remote_repo_path is None:
        ...
        git_remote_url = git_remote_url_part0 + remote_repo_path
    quoted_git_remote_url = single_quote_it(git_remote_url)
    log.info( "ADDING REMOTE ENTRY to git repo: " + git_remote_url )
    log.info( "quoted git remote url : " + quoted_git_remote_url )
    fish_script_pre = gitaddremotecmd+' '+git_remote_entry_name+' '+quoted_git_remote_url
    fish_script = prepend_goto_dir_to_fishcode(fish_script_pre,repopath)
    log.debug('running fish script')
    log.debug(fish_script)
    retcode = call_fish_shellscript(fish_script)
    log.debug(retcode)
    if retcode == 128 or retcode==0:
...
def scan_file_and_check(filename):
    lines=scan_file(filename)
    # check for .git dir
    liste_local =[]
    liste_remote=[]
    for line in lines:
...
def prepare_path_liste(liste):
    ...
    for i in liste:
        ...
        for i in range(1,100000):
            if i%100 == 0:
                 log.warn("very deep path or bug in software")
            sp=path.split(p)
            pp=[sp[1]] + pp
            p=sp[0]
            if sp[0]=='/':
    ...
    for i in ppp:
        if pp[0] == '/':
            ...
            s = '/'.join(i)
        ll.append( ( i, s ) )   # tuple with list and string
    log.debug(ll)
    return ll

def prepared_path_liste_for_submodule_fix(prepared_list):
    """
    Removes stuff that is not useful for submodule fixing.
    dataformat (in/out): (['pathcomponent','basename_path'],'path'))
...
def __make_url(  local_path , url=None , cut_path=0 , shorten_remote_path=True ,urlpostfix=""):
    if not cut_path == 0:
        local_path = cut_path_components( local_path, cut_path )
    if url is None:
        return './' + local_path
    else:
        ...
        if True:
            if shorten_remote_path:
                ...
                return url  + local_path + urlpostfix
        else:
            raise Exception('url invalid')
        
def cut_path_components(p,count):
        sep='/'
        return sep.join(p.split(sep)[count:])
    
def submodule_fix(url=None,pathfile=None, shorten_remote_path=True,urlpostfix=""):
    l=get_repopath_liste() 
    l=prepared_path_liste_for_submodule_fix(l)
...
def get_repopath_liste():
    """
    in/out: ([pathp,pathp2],path)
    """
    global repopathliste
    if repopathliste is None:
        if not args.file==None:
            l = scan_file(args.file)
        else:
            l = gen_repopath_liste()
        log.debug("")
        log.debug(l)
        l=prepare_path_liste(l)
    else:
        l=repopathliste
    #for i in l:
    #    print(i)
...
def remote_path_fix(remotename,url=None, pathfile=None, cut_path=0, shorten_remote_path=False,urlpostfix=""):
    l=get_repopath_liste()
    #for i in l:
    #    print(i)
    ...
    for i in l:
           log.debug(i)
           i_url = __make_url(local_path=i[1] , url=url, cut_path=cut_path, shorten_remote_path=shorten_remote_path , urlpostfix=urlpostfix)
           p = i[1]
...
def prepare_path(p,cut_path=0,url='',postfix='',shorten_path=False):
          source=p
          if cut_path is None:
                  if shorten_path:
                       target = url + __shorten_path(path.basename(source)) + postfix
                  else:
                       target = url + path.basename(source) + postfix
          else:
                  d = cut_path_components(source,cut_path)
                  if shorten_path:
                       target = url + __shorten_path(d) + postfix
                  else:
                       target = url + d + postfix
          return target

def do_cmd(cut_path=None,url='',postfix='',shorten_path=False):
    l=get_repopath_liste()
    log.debug("")
    log.debug(l)
    for d in l:
          d = prepare_path(d[1],cut_path=cut_path,url=url,postfix=postfix,shorten_path=shorten_path)
          cmd = args.cmd
          cmd = prepend_goto_dir_to_fishcode(cmd , d )
          if args.verbose or args.dry_run:
              print(cmd)
          if not args.dry_run:
              call_fish_shellscript(cmd)
        
def do_clone(url, cut_path=None , postfix='' ,shorten_path=False,branch=None):
    l=scan_file(args.file)
    for source in l:
          target = prepare_path(source,cut_path=cut_path,url=url,postfix=postfix,shorten_path=shorten_path)
          if branch is not None:
              cmd = 'git clone -b ' +branch+ ' -- '+source+' '+target
          else:
              cmd = 'git clone -- '+source+' '+target
          createdir = not path.exists(target)
          if args.verbose or args.dry_run:
              print()
              if createdir and args.mkdir:
                  print("would create path: "+ target)
              print(cmd)
          if not args.dry_run:
              if createdir and args.mkdir:
                   makedirs(target)
...
class Base_prefixed_path():
    """
    Represents a path that consists of a basepath and a subpath.
    So base and or subpath can easily exchanged.
    Also supports useful path manipulations that are related
    to the basepath-subpath data thing.
    """
    class Shiftlevel_error(Exception):
        pass

    @staticmethod
    def _process_path_(path,expandvars=False,expanduser=False):
        """preprocess the pathes"""
        path=normpath(path)
        if expandvars:
            path=f_expandvars(path)
        if expanduser:
            path=f_expanduser(path)
        return path

    def __init__(self,basepath,subpath,subpath_includes_base=False,expanduser=False,expandvars=False):

        pathes={'basepath':basepath, 'subpath':subpath}
        for p,v in pathes.items():
            pathes[p]=self._process_path_(v,expandvars=expandvars,expanduser=expanduser)

        self.basepath=pathes['basepath']

        if not subpath_includes_base:
            self.subpath=pathes['subpath']
        else:
            if pathes['subpath'].find(pathes['basepath']) == 0:
                self.subpath=subpath[len(pathes['basepath']):].strip(psep)
            else:
                raise Exception (
                                    "ERROR: basepath not included in subpath, as declared by arguments.\n"
                                    +"in this case, subpath is actually no subpath.\n"
                                    +"basepath="+str(pathes['basepath'])+"\n"
                                    +"subpath="+str(pathes['subpath'])+"\n"
                                    +"path_includes_base="+str(path_includes_base)
                                )
    def __repr__(self):
        return self.__class__.__name__+"('"+self.basepath+"','"+self.subpath+"')"
    ...
    def get_path(self):
        ...
        """
        return self._process_path_(self.basepath+(psep if not self.basepath=="/" else "")+self.subpath)

    def set_subpath(self,subpath,expanduser=False,expandvars=False):
        self.subpath=self._process_path_(subpath,expandvars=expandvars,expanduser=expanduser)

    def set_basepath(self,basepath,expanduser=False,expandvars=False):
        self.basepath=self._process_path_(basepath,expandvars=expandvars,expanduser=expanduser)

    def set_fullpath(self,fullpath,expanduser=False,expandvars=False):
        fullpath=self._process_path_(fullpath)
        if self.basepath in ("","."):
            self.subpath=fullpath
            return
        if fullpath.find(self.basepath) == 0:
            self.subpath=fullpath[len(self.basepath):].strip(psep)
            return
        if self.subpath in ("","."):
            self.basepath=fullpath
            return
        pos=fullpath.find(self.subpath)
        if pos!=-1:
            basepath=fullpath[:pos].strip(psep)
            return
        raise Exception (
                            "ERROR: could not extract information from the supplied path.\n"
                            + "Pathes are stored as basepath and subpath.\n"
                            + "As there is no direct storage, a splitpoint need to be inferable.\n"
                            +"supplied_full_path='"+str(fullpath)+"'\n"
                            +"basepath='"+str(self.basepath)+"'\n"
                            +"subpath='"+str(self.subpath)+"'\n"
                        )

    def shift_boundary(self,shiftlevel):
        """
        positiv shiftlevel shifts the boundary between basepath and subpath
        so that basepath becomes longer and the subpath gets shortened.
        """
        splitted_basepath=sub('^[.][/]','',self.basepath.strip(psep)).split(psep)
        splitted_subpath=sub('^[.][/]','',self.subpath.strip(psep)).split(psep)
        splitted_basepath= [] if splitted_basepath == [''] else splitted_basepath
        splitted_subpath= [] if splitted_subpath == [''] else splitted_subpath
        baselen=len(splitted_basepath)
        sublen=len(splitted_subpath)
        maxlevel= sublen
        minlevel = - baselen
        if shiftlevel=="max":
            shiftlevel=maxlevel
            ...
            shiftlevel=minlevel
        if shiftlevel > 0 :
            if shiftlevel > maxlevel:
                raise self.Shiftlevel_error("ERROR: shiftlevel is to high, maxlevel is "+str(maxlevel)+" .")
            new_splitted_basepath = splitted_basepath+splitted_subpath[:shiftlevel]
            new_splitted_subpath  =                   splitted_subpath[shiftlevel:]

            
        elif shiftlevel < 0 :
            if shiftlevel < minlevel:
                raise self.Shiftlevel_error("ERROR: shiftlevel is to low, minlevel is "+str(minlevel)+" .")
            new_splitted_basepath  = splitted_basepath[:shiftlevel]
            new_splitted_subpath   = splitted_basepath[shiftlevel:]+splitted_subpath
        else:
            return
        basepath=psep.join(new_splitted_basepath)
        basepath= "/"+ basepath if ( len(self.basepath) > 0 and  self.basepath[0] == "/") else  basepath
        subpath=psep.join(new_splitted_subpath)

        self.basepath=normpath(basepath)
        self.subpath=normpath(subpath)

    def _selftest_(self):
        from random import choice as random_choice
        subpathes='a/b/c','','.','./','a','abc','a/.','a/b/c/.','a/b/c/'
        basepathes = subpathes +( "/","/.")
        pathtuples=[]
        def check2(p1,p2):
            if not str(p1)==str(p2)\
                   or not p1.basepath == p2.basepath\
                   or not p1.subpath == p2.subpath\
                   or not p1.get_path() == p2.get_path():
                       raise Exception  (
                                            "Test failed\n"
                                            +"str(p1)="+str(p1)+"\n"
                                            +"str(p2)="+str(p2)+"\n"
                                            +"repr(p1)="+str(p1)+"\n"
                                            +"repr(p2)="+str(p2)+"\n"
                                            +"p1.get_path()="+p1.get_path()+"\n"
                                            +"p2.get_path()="+p2.get_path()+"\n"
                                            +"p1.basepath="+p1.basepath+"\n"
                                            +"p2.basepath="+p2.basepath+"\n"
                                            +"p1.subpath="+p1.subpath+"\n"
                                            +"p2.subpath="+p2.subpath+"\n"
                                        )
        ...
        for i in basepathes:
            for j in subpathes:
                pathtuples+=[(i,j)]
        for t in pathtuples:
            print('testing pathtuple:',t,end=' ')
            p=self.__class__(t[0],t[1])
            p1=self.__class__(t[0],t[1])
            check2(p,p1)
            upshiftcount=0
            while True:
                try:
                    p1.shift_boundary(1)
                    upshiftcount+=1
                except self.Shiftlevel_error:
                    break
            for i in range(upshiftcount):
                try:
                    p1.shift_boundary(-1)
                except self.Shiftlevel_error:
            ...
            p1.shift_boundary(0)
            check2(p,p1)
            p2=self.__class__(p.basepath,p.subpath)
            for i in range(100):
                subpath=random_choice(subpathes)
                p1.set_fullpath(p1.basepath+psep+subpath)
                p2.set_fullpath(p2.basepath+psep+subpath)
                check2(p1,p2)
                if not path.isabs(p1.basepath):
                    basepath=random_choice(basepathes)
                    p1.set_fullpath(basepath+psep+p1.subpath)
                    p2.set_fullpath(basepath+psep+p2.subpath)
                check2(p1,p2)
            print(" ok")
...
if __name__=="__main__":
    ...
    if args.remote_path_fix:
        remote_path_fix(args.remote_name,url=args.url,urlpostfix=args.urlpostfix,pathfile=args.file ,shorten_remote_path=args.shorten_remote_path,cut_path=args.cut_path)
    ...
    if args.cmd is not None:
        do_cmd(cut_path=args.cut_path,url=args.url,postfix=args.urlpostfix)
    if args.clone:
        do_clone( args.url , cut_path=args.cut_path,postfix=args.urlpostfix, shorten_path = args.shorten_remote_path ,branch=args.branch)
# vim: set syntax=python foldlevel=0 foldmethod=indent          :
